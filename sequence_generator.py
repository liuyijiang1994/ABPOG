...
class SequenceGenerator(object):
    ...
    def beam_search(self, src, src_lens, src_mask, word2idx, aspect_id, max_eos_per_output_seq=1):
        """
        ...
        # Encoding
        memory_bank, encoder_final_state, asp_hidden = self.model.predict(src, src_lens, src_mask, aspect_id)
        ...
        aspect_id = aspect_id.repeat(beam_size)
        asp_hidden = asp_hidden.repeat(beam_size, 1)
        decoder_state = decoder_init_state.repeat(self.beam_size, 1)
        decoder_init_state = decoder_init_state.repeat(self.beam_size, 1)
        # [dec_layers, batch_size * beam_size, decoder_size]
        ...
        sentiment_state = torch.zeros((batch_size * beam_size)).to(memory_bank.device)
        weighted_asp_hidden = torch.zeros(batch_size * beam_size, memory_bank.size()[-1]).to(memory_bank.device)
        ...
        for t in range(1, self.max_sequence_length + 1):
            ...
            # [flattened_batch, vocab_size], [flattened_batch, decoder_size], [flattened_batch, memory_bank_size], [flattened_batch, src_len], [flattened_batch, src_len]
            decoder_dist, final_dist, decoder_state, context, \
            attn_dist, p_gen, coverage, sentiment_state, weighted_asp_hidden = \
                self.model.decoder(decoder_input, decoder_state, memory_bank, src_mask,
                                   src, aspect_id, asp_hidden,
                                   decoder_init_state, sentiment_state, coverage, weighted_asp_hidden)
    ...
    def beam_decoder_state_update(self, batch_idx, beam_indices, decoder_state, decoder_memory_bank=None):
        ...
        :param decoder_state: [ flattened_batch_size, decoder_size]
        :return:
        """
        flattened_batch_size, decoder_size = list(decoder_state.size())
        ...
        decoder_state_transformed = decoder_state.view(self.beam_size, original_batch_size,
                                                       decoder_size)[:, :, batch_idx]
        # select the hidden states of the beams specified by the beam_indices -> [ beam_size, decoder_size]
        decoder_state_transformed.data.copy_(decoder_state_transformed.data.index_select(0, beam_indices))
    ...
    def sample(self, src, src_lens, src_oov, src_mask, oov_lists, max_sample_length, greedy=False, one2many=False,
               one2many_mode=1, num_predictions=1, perturb_std=0, entropy_regularize=False, title=None, title_lens=None,
               title_mask=None):
